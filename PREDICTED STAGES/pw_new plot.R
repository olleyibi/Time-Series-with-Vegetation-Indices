anth=emergence=ff=har=pi=pw=ms=max_dv=min_dv=space=max_gndvi=vector()
...
n = length(fld)
m=0
pdf("gndvi_derivative3.pdf", onefile = TRUE)
for(i in seq(ceiling(n/4))) {
  ...
  for(j in 1:k) {
    ...
    # GAM smoothing model
    mod = gam(gndvi~s(day_dif,bs='cr'),data=fd) # using cubic regression spline
    ...
    fd57=fd[which(fd$day_dif>100 & fd$day_dif<180),]
    max_dv[m]=as.data.frame(fd57[fd57[,which(fd57$der == max(fd57$der))],29])[1,1]
    
    maxdv[m]=max(fd$der)
    mindv[m]=min(fd$der)
    
    #max_dv[m]=as.data.frame(fd[fd[,which(fd$der == max(fd$der))],29])[1,1]
    max_gndvi[m]=as.data.frame(fd[fd[,which(fd$pred == max(fd$pred))],29])[1,1]
    ...
    plot_list[[j]] <- ggplot(fd, aes(day_dif))+
      ...
      geom_line(aes(y=pred),col='red') + # GAM gndvi
      geom_line(aes(y=der*scaleFactor),col='blue')+ # first derivative
      geom_vline(xintercept = vert['PW','diff'],col='red') +
      scale_y_continuous(name="gndvi", sec.axis=sec_axis(~./scaleFactor, name="Deriv(gndvi)"))+
      theme(axis.title.y.left=element_text(color="red"),
            axis.title.y.right = element_text(color="blue"))+
      annotate(x =vert['PW','diff'], y = 0.5, label = 'PW',geom='text',
...
ggplot(try,aes(x=actual,y=predict))+
  ...
  annotate(geom='text',x=min(try$actual)+10,y=max(try$predict)-20,
           label=paste("RMSE:",round(RMSE(try$predict,try$actual),3)),col='blue')+
  ggtitle("RMSE value for PW using GAM(max deriv) gndvi")
