anth=emergence=ff=har=pi=pw=ms=max_dv=min_dv=space=max_grvi=vector()
...
n = length(fld)
m=0
pdf("grvi_derivative3.pdf", onefile = TRUE)
for(i in seq(ceiling(n/4))) {
  ...
  for(j in 1:k) {
    ...
    # fn = as.data.frame(tapply(fd$grvi,fd$day_dif,mean))
    # fn <- cbind(day_diff = rownames(fn),grvi = fn)
    # rownames(fn) <- 1:nrow(fn)
    # colnames(fn) <- list("day_diff","grvi")
    ...
    # GAM smoothing model
    mod = gam(grvi~s(day_dif,bs='cr'),data=fd) # using cubic regression spline
    ...
    max_grvi[m]=as.data.frame(fd[fd[,which(fd$pred == max(fd$pred))],29])[1,1]
    ...
    plot_list[[j]] <- ggplot(fd, aes(day_dif))+
      ...
      geom_line(aes(y=pred),col='red') + # GAM grvi
      geom_line(aes(y=der*scaleFactor),col='blue')+ # first derivative
      geom_vline(xintercept = vert['Harvest','diff'],col='red') +
      scale_y_continuous(name="grvi", sec.axis=sec_axis(~./scaleFactor, name="Deriv(grvi)"))+
      theme(axis.title.y.left=element_text(color="red"),
            axis.title.y.right = element_text(color="blue"))+
      annotate(x = vert['Harvest','diff'], y = 0.5, label = 'Harvest',geom='text',
...
ggplot(try,aes(x=actual,y=predict))+
  ...
  ggtitle("RMSE value for Harvest using GAM(min deriv) grvi")



max_derv=round(RMSE(try$predict2,try$actual),3)
max_vi=round(RMSE(try$predict1,try$actual),3)
min_derv=round(RMSE(try$predict,try$actual),3)
